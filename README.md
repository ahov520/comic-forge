...
- **域名黑名单**：设置页维护本机拦截列表；浏览 / 搜索 / 阅读图片 / 下载会拒绝匹配主机及其子域，已缓存的离线内容仍可阅读
...
- **按漫画统计**：阅读统计页下钻，按最近阅读查看每本漫画的累计时长、去重话数、阅读次数和最近阅读时间；书架长按漫画或详情页「阅读统计」直达该书数据，无记录时显示阅读引导。同名漫画按来源区分，重复进入章节累加次数，暂停续读不重复计次。旧版累计统计保留，单本明细从升级后开始记录
