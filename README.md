...
- **源健康**：搜索/发现失败自动回报，连续失败≥3 源页标红；源管理可按健康/停用筛选，并一键禁用或重新启用失效源
- **聚合搜索**：并发查启用源，可按健康状态或指定源组合过滤；筛选与「最近10词」本地保留，支持点按回填与一键清空，切换标签保留结果和草稿；结果卡可一键加入书架，已在书架时可移出或打开该书
- **沉浸阅读器**：黑底与毛玻璃工具栏、滚动/翻页双模式（翻页支持双指缩放）、话间导航、章内页进度/跳页和亮度调节；失败图片可独立重试；可把当前阅读偏好存成命名预设，设置页或阅读器内一点切换
...
- **批量离线下载**：详情页选择多话或选中未读章节，队列显示图片进度并支持失败补传；书架菜单和设置页可进入下载管理，完成的漫画可跨重启离线阅读。Android 可查看离线与图片缓存占用，确认后清除已完成/失败任务、按漫画删除离线文件，或清除在线图片缓存（不影响保留的离线章节）
- **阅读历史**：按今天、昨天和日期查看最近阅读，可按漫画名、章节或来源即时搜索，支持直接续读和移除记录；未收藏漫画同样保留，旧版进度自动迁移，移除记录不会清除真实阅读进度
- **继续阅读**：Android 书架顶部展示最近阅读条（封面 + 章节/进度），点按走现有续读；无记录时不占位，不干扰分组、搜索与排序
